...
int main() {
  ...
  for (j = 0; j < counter; j++) {
    printf("       %lld   %.2f         %d       %.2f\n", item_Barcode[j], item_Price[j], item_Quantity[j], item_Value[j]);
  }
  printf("                                         ------\n");
...
void array(long long a[], int i) {
  ...
  while (valid == 0) {
    printf("Barcode: ");
    scanf("%lld", & a[i]);
    if ((a[i] > 9999999999 || a[i] < 1000000000) && a[i] != 0)
